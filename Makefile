...
.PHONY: clean-docs
clean-docs: setup-venv   ## Remove build artifacts for documentation only
...
clean-pkgbuild:    ## Remove build artifacts for documentation only
...
.PHONY: clean
clean:  setup-venv clean-docs clean-version clean-pkgbuild  ## Remove all build artifacts generated by make
	@echo "Clean project directories"
	sudo chown -R $(USER):$(USER) $(CUR_DIR)/$(PROJECT)
	sudo rm -rf $(VENV_DIR) || true
	sudo rm -rf $(CUR_DIR)/dist || true
	sudo rm -rf $(CUR_DIR)/*.egg* || true
	sudo rm -f $(CUR_DIR)/README.rst || true
	sudo rm -rf $(CUR_DIR)/$(PROJECT)/__pycache__ || true
	sudo rm -rf $(CUR_DIR)/tests/__pycache__ || true
	sudo rm -rf $(CUR_DIR)/docs/__pycache__ || true
	sudo rm -rf $(CUR_DIR)/.pytest_cache || true
	sudo rm -rf $(CUR_DIR)/build || true
