...
##     - Purpose:  Perform configuration steps, Docker context
##     - Author:   Blake Huber
##     - Log:      /home/builder/logs/dockerbuild.log
##     - Caller:   buildrpm.py (host)
##
##  Copyright 2017-2019 Blake Huber.  All rights reserved.
##
...
function rpm_contents(){
    ...
    ##
    local rpmfile="/home/builder/rpmbuild/RPMS/noarch/xlines-*.rpm"
...
function std_message(){
    ...
    if [ $format ]; then
        ...
        echo -e "${yellow}[ $cyan$pref$yellow ]$rst  $msg" | indent04
...
function std_error(){
    ...
    echo -e "\n${yellow}[ ${red}ERROR${yellow} ]$rst  $msg\n" | indent04
    #
...
function std_warn(){
    ...
    if [ "$3" ]; then
        ...
        echo -e "\n${yellow}[${rst} ${byl}WARN${yellow} ]$rst  $msg" | indent04
    else
        # msg is only 1 line sent by the caller
        echo -e "\n${yellow}[${rst} ${byl}WARN${yellow} ]$rst  $msg\n" | indent04
    fi
    #
