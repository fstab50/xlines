...
if [ "$1" ]; then
    ...
    \n${tab}$ sh $pkg 'xlinesUbuntu18'" "FAIL"
    exit 1
...
# working directory
cd "$(_git_root)/packaging/docker/ubuntu18" || false
...
docker run -it --user='builder' --name=$container -d -v /tmp/deb:/mnt/deb $image tail -f /dev/null &
